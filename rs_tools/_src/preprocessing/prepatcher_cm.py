...
def _check_nan_count_cm(arr: np.array, nan_cutoff: float) -> bool:
    """
    Check if the number of NaN values in the given array is below a specified cutoff.
    ...
    """
    # count -1 in cm
    nan_count =  np.count_nonzero(arr[11] == -1) 
    # get total pixel count
    total_count = int(arr[11].size)
    # check if nan_count is within allowed cutoff
    ...
    def save_patches(self):
        ...
        for ifile in pbar:
            ...
            for i, ipatch in tqdm(enumerate(patcher), total=len(patcher)):
                ...
                if _check_nan_count(data, self.nan_cutoff) and _check_nan_count_cm(data, nan_cutoff = 0.2):
...
if __name__ == "__main__":
    ...
    """
    prepatch(read_path = '/mnt/data8tb/fire_detection/modis/msg_cm_dataset', save_path='/mnt/data8tb/fire_detection/modis/msg_cm_dataset_patches', patch_size=32, stride_size=32, nan_cutoff=0.4, save_filetype='tif')
    #typer.run(prepatch)
